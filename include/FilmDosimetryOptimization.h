...
class FilmDosimetryOptimization {
  ...
  private:
    ...
    :return: Dose ( cGy )
    */
    double inverseEquation(double od, int eqt, std::vector<double> p);

    double diffCalibrationEquation(double x, int eqt, std::vector<double> p);
    /*
    ...
    */
    double objectiveFunction(double dz, cv::Vec3d od, std::vector<int> eqt,
                             std::vector<double> pol);
    /*
    ...
    */
    /*
        Calculate dose using robust MC - optimization
    */
    bool cmpf(double A, double B, double epsilon = 2.2204460492503131e-16) {
