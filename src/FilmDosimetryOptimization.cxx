...
*/
double FilmDosimetryOptimization::inverseEquation(double od, int eqt,
                                                  std::vector<double> p) {
...
*/
double
FilmDosimetryOptimization::diffCalibrationEquation(double x, int eqt,
                                                   std::vector<double> p) {
    double A = 100.0;
    double B = 500.0;

    if (eqt == 1) {
        return (2 * p[2] * log((A + x) / A) + p[1]) / (A + x);
    }
    if (eqt == 2) {
        return 2 * p[2] * x + p[1];
    }
    if (eqt == 3) {
        return ((2 * p[2] * atan(x / B) + p[1]) * B) / (pow(B, 2) + pow(x, 2));
    }
    return 0.;
}
...
*/
double FilmDosimetryOptimization::objectiveFunction(double dz, cv::Vec3d od,
                                                    std::vector<int> eqt,
                                                    std::vector<double> pol) {
    /*
     * Helper Derivative of Objective function to find equation roots
            ...
            if (cmpf(fx, 0.)) {
                ...
                while (!cmpf(fb, 0.) && !cmpf(a, b)) {
                    ...
                    if (tt1 || tt2) {
                        ...
                        auto t1 =
                            2.0 * p < (3.0 * m * q - std::fabs(toler * q));
